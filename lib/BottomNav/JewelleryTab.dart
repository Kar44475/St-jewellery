...
class _JewelleryTabState extends State<JewelleryTab> {
  ...
  void initState() {
    super.initState();
    // Don't call _loadJewelleryData() here
  }

  @override
  void didChangeDependencies() {
    super.didChangeDependencies();
    // Call the data loading method here instead
    if (!_hasInitialized) {
  ...
  Widget _buildCategoryFilter() {
    return Column(
      ...
      children: [
        ...
        SizedBox(height: 15),
        // Fixed container height and padding to accommodate shadows
        Container(
          height: 70, // Increased height to accommodate shadows
          child: SingleChildScrollView(
            scrollDirection: Axis.horizontal,
            padding: EdgeInsets.symmetric(horizontal: 15, vertical: 10), // Added vertical padding
            child: Row(
  ...
  Widget _buildCategoryChip(String title, int index, IconData icon) {
    ...
    return GestureDetector(
      ...
      child: Container(
        // Added margin to prevent shadow clipping
        ...
        child: AnimatedContainer(
          ...
          decoration: BoxDecoration(
            ...
            // Reduced shadow intensity and spread
            boxShadow: [
              BoxShadow(
                color: isSelected 
                ...
                blurRadius: isSelected ? 6 : 4, // Reduced blur
                offset: Offset(0, isSelected ? 2 : 1), // Reduced offset
              ),
            ],
            border: Border.all(
              color: isSelected
  ...
  // Load jewellery data - Fixed to avoid context issues
  Future<void> _loadJewelleryData() async {
    try {
      // Don't show loading dialog in initState, just set loading state
      setState(() {
        isDataLoaded = false;
      });
  ...
  // Show error message using SnackBar instead of dialog
