...
import 'package:stjewellery/screens/Recepit/Recepit.dart'; // Add this import
...
class _PaymentHistoryScreenState extends State<PaymentHistoryScreen> {
  ...
  Future<void> _loadPaymentDetails() async {
    try {
      ...
      Paymentdetailsmodel paymentDetailsData = await Paymentdetailsservice.postService(details);
...
Icons.person,
                ...
                // Updated View Receipt button - only show for completed payments
                Visibility(
                //  visible: _isPaymentCompleted(payment),
                  child: Container(
                    decoration: BoxDecoration(
  ...
  bool _isPaymentCompleted(dynamic payment) {
    try {
      return payment.status == 2; // Only show receipt for completed payments
    } catch (e) {
  ...
  // Proper receipt implementation following ViewAllPaymentHistory pattern
  void _viewReceipt(dynamic payment) {
    try {
      // Get payment ID - this is the key part from ViewAllPaymentHistory
      var paymentId = payment.sheduledDateId;
      
      if (paymentId != null) {
        // Navigate to Receipt screen with payment ID as argument
        // This follows the exact pattern from ViewAllPaymentHistory.dart
        Navigate.push(
          context,
          Receipt(arguments: paymentId),
        );
      } else {
  ...
  // Alternative method if you want to show all payment history like ViewAllPaymentHistory
  void _viewAllPaymentHistory() {
    if (data?.data?.paidPayment != null) {
      ...
      Navigate.push(
        context,
        ViewAllPaymentHistory(arguments: details)
      );
    }
  }
}
